...
# 创建ASP专属功能蓝图
asp_shell_bp = Blueprint('asp_shell', __name__, url_prefix='/api/webshell/asp')


@asp_shell_bp.route('/executeCommand', methods=['POST'])
def execute_command():
    """执行webshell命令"""
    try:
        data = request.json
...
@asp_shell_bp.route('/getFiles', methods=['POST'])
def get_files():
    """获取指定目录的文件列表"""
    try:
        data = request.json
        ...
        return jsonify({'status': 'success', 'data': {'path': path, 'files': file_list}})
...
@asp_shell_bp.route('/uploadFile', methods=['POST'])
def upload_file():
    """上传文件（支持普通上传和大文件上传）"""
    try:
        data = request.json
            ...
            return jsonify(
                {
                    'status': 'success',
                    'message': '文件上传成功',
                    'data': {'filePath': file_path, 'uploadMode': 'normal'},
                }
            )
...
@asp_shell_bp.route('/downloadFile', methods=['POST'])
def download_file():
    """下载文件（支持普通下载和大文件下载）"""
    try:
        data = request.json
...
@asp_shell_bp.route('/deleteFile', methods=['POST'])
def delete_file():
    """删除文件"""
    try:
        data = request.json
            ...
            return jsonify(
                {
                    'status': 'success',
                    'message': '文件删除成功',
                    'data': {'fileName': file_name, 'result': 'success'},
                }
            )
...
@asp_shell_bp.route('/newFile', methods=['POST'])
def new_file():
    """新建文件"""
    try:
        data = request.json
...
@asp_shell_bp.route('/newDir', methods=['POST'])
def new_dir():
    """新建目录"""
    try:
        data = request.json
            ...
            return jsonify(
                {
                    'status': 'success',
                    'message': '目录创建成功',
                    'data': {'dirName': dir_name, 'result': 'success'},
                }
            )
...
@asp_shell_bp.route('/copyFile', methods=['POST'])
def copy_file():
    """复制文件"""
    try:
        data = request.json
...
@asp_shell_bp.route('/moveFile', methods=['POST'])
def move_file():
    """移动文件"""
    try:
        data = request.json
...
@asp_shell_bp.route('/zip', methods=['POST'])
def zip_files():
    """压缩文件"""
    try:
        data = request.json
            ...
            return jsonify(
                {
                    'status': 'success',
                    'message': '文件压缩成功',
                    'data': {'compressPaths': compress_paths, 'compressFile': compress_file},
                }
            )
...
@asp_shell_bp.route('/unzip', methods=['POST'])
def unzip_files():
    """解压文件"""
    try:
        data = request.json
            ...
            return jsonify(
                {
                    'status': 'success',
                    'message': '文件解压成功',
                    'data': {'compressFile': compress_file, 'extractDir': extract_dir},
                }
            )
...
@asp_shell_bp.route('/fileRemoteDownload', methods=['POST'])
            ...
            return jsonify(
                {
                    'status': 'success',
                    'message': '远程文件下载成功',
                    'data': {'downloadUrl': download_url, 'savePath': save_path},
                }
            )
...
@asp_shell_bp.route('/setFileAttr', methods=['POST'])
def set_file_attr():
    """设置文件属性（权限或时间）"""
    try:
        data = request.json
        ...
        if attr_type == 'permission':
            if not hasattr(shell, 'set_file_permission'):
                return jsonify({'status': 'error', 'message': '当前webshell类型不支持文件权限设置'}), 400
        ...
        for time_value, field_name in [
            ...
            if time_value and str(time_value).strip():
                try:
                    ...
                    return (
                        jsonify({'status': 'error', 'message': f'{field_name}格式无效，请使用：YYYY-MM-DD HH:MM:SS'}),
                        400,
                    )
            else:
...
@asp_shell_bp.route('/testDatabaseConnection', methods=['POST'])
...
@asp_shell_bp.route('/executeSql', methods=['POST'])
def execute_sql():
    """执行SQL语句"""
    try:
        data = request.json
