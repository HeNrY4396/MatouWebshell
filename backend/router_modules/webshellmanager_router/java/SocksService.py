...
class SOCKS5Handler:
            ...
            while True:
                ...
                try:
                    ...
                    if client_data:
                        ...
                        # 写入远程隧道
                        success = self.tunnel_service.shell.doWrite(
                            self.socket_hash, 
                            client_data,
                            self.tunnel_service.socks_class_name
                        )
                        ...
                        if not success:
                            print("[DEBUG] 写入远程隧道失败")
                            break
                        
                        has_data_activity = True
                        
                        # 写入成功后，立即尝试读取响应
                        time.sleep(0.01)  # 短暂等待服务器处理
                ...
                # 步骤2：尝试从远程读取数据并发送给客户端
                try:
                    remote_data = self.tunnel_service.shell.doRead(
                        self.socket_hash,
                        self.tunnel_service.socks_class_name
                    )
                    
                    if remote_data is None:
                        print("[DEBUG] 远程隧道读取失败或连接关闭")
                        # 如果有写入活动但读取失败，可能是暂时性问题，继续尝试
                    ...
                    if "socketchannel closed by peer" in error_msg:
                        print("[DEBUG] 远程连接被对端关闭，这是正常的HTTP连接行为")
                        # 对于HTTP连接，服务器完成响应后关闭连接是正常的
                        # 如果已经有数据活动，说明请求已处理，可以继续
                        if has_data_activity:
                            continue
                        else:
                            # 如果没有数据活动，可能需要等待或重试
                            pass
                    elif "socket connection not found" in error_msg:
                        print("[DEBUG] 连接不存在，可能需要重新创建")
                        # 这种情况下可以尝试继续，doRead会自动重新创建连接
                        continue
                    else:
                        print(f"[DEBUG] 远程读取异常: {e}")
                        # 其他异常，继续循环，可能是暂时性错误
    ...
    def forward_client_to_remote(self):
        """转发客户端数据到远程服务器"""
        try:
            while True:
                # 从本地客户端读取数据
                ...
                print(f"[DEBUG] 客户端->远程: 接收 {len(data)} 字节")
            ...
            print(f"[ERROR] 客户端->远程转发异常: {e}")
        finally:
            print("[DEBUG] 客户端->远程转发线程结束")
    
    def forward_remote_to_client(self):
        """转发远程服务器数据到客户端"""
        try:
            consecutive_empty_reads = 0
            max_empty_reads = 50  # 最多连续50次空读取
            
            while True:
                # 从远程隧道读取数据
                data = self.tunnel_service.shell.doRead(
                    self.socket_hash,
                    self.tunnel_service.socks_class_name
                )
                ...
                if data is None:
                    ...
                    if consecutive_empty_reads >= max_empty_reads:
                        print("[DEBUG] 连续空读取次数过多，可能连接已断开")
                        break
                    
                    # 渐进式等待时间，避免过于频繁的轮询
                    wait_time = min(0.1 + consecutive_empty_reads * 0.02, 0.5)
                ...
                print(f"[DEBUG] 远程->客户端: 接收 {len(data)} 字节")
            ...
            print(f"[ERROR] 远程->客户端转发异常: {e}")
        finally:
            print("[DEBUG] 远程->客户端转发线程结束")
...
class SocksService:
    ...
    def __init__(self, shell: JavaShell, socks_class_name ,ip_whitelist=None):
        self.shell = shell
...
def test_tunnel_service_jsp():
    ...
    try:
        ...
        classInfo = payload.getPayload(
            class_name="SocksProxy", 
            obfuscation_method='none'
        )
        
        if not shell.include(classInfo):
            print("❌ 加载SocksProxy类失败")
            return False

        # 创建隧道服务
        tunnel_service = SocksService(shell, IP_WHITELIST,socks_class_name=classInfo["classname"])
